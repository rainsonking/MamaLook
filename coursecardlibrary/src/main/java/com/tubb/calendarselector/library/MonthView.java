...
public class MonthView extends FrameLayout {
    ...
    private List<String> noAttentDays = new ArrayList<>();//没有上课天
    private List<String> attentDays = new ArrayList<>();//已上课天
    private List<String> leaveAttentDays = new ArrayList<>();//请假天
    ...
    public MonthView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (isInEditMode()) {
            ...
            String attendDays = a.getString(R.styleable.MonthView_sc_attend_days);
            String noAttendDays = a.getString(R.styleable.MonthView_sc_no_attend_days);
            String leaveDays = a.getString(R.styleable.MonthView_sc_leave_days);
//            initEditorMode(testMonth, selectedDays);
     ...
     * @param scMonth
     * @param dayInflater
     * @param noAttentDays    没有上课天
     * @param attentDays      已上课天
     * @param leaveAttentDays 请假天
     */

    public void setSCMonth(final SCMonth scMonth, DayViewInflater dayInflater, final List<String> noAttentDays, final List<String> attentDays, final List<String> leaveAttentDays) {
        ...
        this.noAttentDays = noAttentDays;
        this.attentDays = attentDays;
        this.leaveAttentDays = leaveAttentDays;

        Log.e(TAG, "setSCMonth: day " + noAttentDays.toString());
        calculateDays();
        if (getChildCount() > 0) {
            refresh();
        } else {
            post(new Runnable() {
                @Override
                public void run() {
//                    createDayViews(noAttentDays, attentDays, leaveAttentDays);
                    createDayViews();
                }
            });
        }
    ...
    //    private void createDayViews(List<String> noAttentDays, List<String> attentDays, List<String> leaveAttentDays) {
...
//                drawDays(row, col, dayView, noAttentDays, attentDays, leaveAttentDays);
    ...
    //    private void drawDays(final int row, final int col, View dayView, List<String> noAttentDays, List<String> attentDays, List<String> leaveAttentDays) {
...
//                setPerMonthDay(dayView, fullDay, dayViewHolder, isSelected, noAttentDays, attentDays, leaveAttentDays);
                setPerMonthDay(dayView, fullDay, dayViewHolder, isSelected);
            }
        } else {
...
//                setPerMonthDay(dayView, fullDay, dayViewHolder, isSelected, noAttentDays, attentDays, leaveAttentDays);
    ...
    /**
     * 没有课3，请假2、已上课时1、未上课时0
     *
     * @param dayView
     * @param fullDay
     * @param dayViewHolder
     * @param isSelected    //     * @param noAttentDays 没有上课天
     *                      //     * @param attentDays 已上课天
     *                      //     * @param leaveAttentDays 请假天
     */
//    private void setPerMonthDay(View dayView, FullDay fullDay, DayViewHolder dayViewHolder, boolean isSelected, List<String> noAttentDays, List<String> attentDays, List<String> leaveAttentDays) {
    private void setPerMonthDay(View dayView, FullDay fullDay, DayViewHolder dayViewHolder, boolean isSelected) {
        String fullDayStr = fullDay.toString();

        Log.e(TAG, "setPerMonthDay: attend  " + attentDays.toString()+" "+fullDayStr);

        if (noAttentDays != null && noAttentDays.size() > 0 && noAttentDays.contains(fullDayStr)) {
            dayView.setVisibility(View.VISIBLE);
            dayViewHolder.setCurrentMonthDayText(fullDay, 0, isSelected);
        } else if (attentDays != null && attentDays.size() > 0 && attentDays.contains(fullDayStr)) {
            dayView.setVisibility(View.VISIBLE);
            dayViewHolder.setCurrentMonthDayText(fullDay, 1, isSelected);
        } else if (leaveAttentDays != null && leaveAttentDays.size() > 0 && leaveAttentDays.contains(fullDayStr)) {
            dayView.setVisibility(View.VISIBLE);
            dayViewHolder.setCurrentMonthDayText(fullDay, 2, isSelected);
        } else {
            dayView.setVisibility(View.VISIBLE);
            dayViewHolder.setCurrentMonthDayText(fullDay, 3, isSelected);
        }
    }
...
//            drawDays(row, col, childView,noAttentDays, attentDays, leaveAttentDays);
     ...
     * @param noAttentDays
     * @param attentDays
     * @param leaveAttentDays
     */
    public void setSCMonth(SCMonth scMonth, List<String> noAttentDays, List<String> attentDays, List<String> leaveAttentDays) {
        this.setSCMonth(scMonth, null, noAttentDays, attentDays, leaveAttentDays);
    }
...
//            }
//        }
//        setSCMonth(scMonth,noAttentDays, attentDays, leaveAttentDays);
//    }
